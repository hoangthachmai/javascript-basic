...
export const TaskProvider = ({ children }) => {
    ...
    const getDocuments = async (url='', documentType='booking', project_id, folder_id, paginateOption = {}) => {
        if (!url) return
        const { page = 1, order_by = 'title', order_type = 'desc', no_item_per_page = 10, career_id = '', category_id = '', search_text = '' } = paginateOption
        axiosInstance.post(url, { page, no_item_per_page, order_by, order_type, category_id, search_text, career_id, folder_id, project_id, outputtype: "RawJson" }).then((res) => {
